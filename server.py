...
def index():
    try:
        ...
        if conn:
            conn.close()
            return jsonify({"message": "Servidor funcionando correctamente. Conexión a BD exitosa."})
        else:
            return jsonify({"message": "Servidor funcionando, pero hay problemas con la conexión a BD."}), 500
